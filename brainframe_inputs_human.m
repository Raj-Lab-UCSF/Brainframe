function input_struct = brainframe_inputs_human(matpath,varargin)
...
load([matpath filesep 'brainframe_defaultHuman_datinput.mat'],...
    'human_atlas_dk','conmat','testdata');
...
%Field to set target directory; default is currect directory
img_directory_ = cd;

%Field to set renderer for saving - opengl is faster but less accurate, 
% painters is slow but better quality
img_renderer_ = 'opengl';
...
validRenderer = @(x) strcmp(x,'opengl') || strcmp(x,'painters');
...
addParameter(ip, 'img_directory', img_directory_, validChar);
addParameter(ip, 'img_renderer', img_renderer_, validRenderer);
addParameter(ip, 'img_views', img_views_, validView);
...
addParameter(ip, 'norm_method', norm_method_, validNorm)
...
input_struct.img_directory = ip.Results.img_directory;
input_struct.img_renderer = ip.Results.img_renderer;
input_struct.img_views = ip.Results.img_views;
...
input_struct.norm_method = ip.Results.norm_method;
