...
%Figure call
figure('Name',input_struct.img_labels);
...
if voxUreg
    ...
    if strcmp(input_struct.norm_method,'mean')
        normvec = scalevec / mean(scalevec);
    elseif strcmp(input_struct.norm_method,'sum')
        normvec = scalevec / sum(scalevec);
    elseif strcmp(input_struct.norm_method,'max')
        normvec = scalevec / max(scalevec);
    elseif strcmp(input_struct.norm_method,'minmax')
...
%                         npts = ceil(normvals(k)*length(pcinds));
    ...
    if iscon
        allcenters = zeros(length(unique(CCFinds))-1,3);
        for g = 1:(length(unique(CCFinds))-1)
            pcinds = find(CCFinds==g);
            [x,y,z] = ind2sub(size(CCFbin),pcinds);
            cent = [mean(y) mean(x) mean(z)];
            allcenters(g,:) = cent;
        end
        meandistxyz = mean(pdist(allcenters));
...
%                     end
                    ...
                    distfun = @(x,y) ((x(:,1)-y(:,1)).^2 + (x(:,2)-y(:,2)).^2 + ...
                        (x(:,3)-y(:,3)).^2).^0.5;
                    distxyz = ((curx(2)-curx(1))^2 + (cury(2)-cury(1))^2 + (curz(2)-curz(1))^2)^0.5; % = 2*length major axis
                    fac = conarch; % perpendicular axis scale factor
                    nt = 200; % define the ellipse; 200 is arbitrary
                    theta_gh = (log(1 + normmat(g,h)) * pi/24)/(distxyz/meandistxyz);
                    theta_hg = (log(1 + normmat(h,g)) * pi/24)/(distxyz/meandistxyz); 
                    narcs_gh = 2 * ceil(log(exp(1) + normmat(g,h))*conscale) + 2; % number of random arcs
                    narcs_hg = 2 * ceil(log(exp(1) + normmat(h,g))*conscale) + 2;
                    rng(0);
...
%                     end
                    ...
                    ellt_dn = gh_on*(repmat(midpt,1,narcs_gh,nt) ...
                                + 0.5*distxyz*repmat(v_,1,narcs_gh,nt).*cos(repmat(t_dn,length(v_),narcs_gh,1)) ...
                                + 0.5*fac*distxyz*repmat(uws_gh,1,1,nt).*sin(repmat(t_dn,length(u),narcs_gh,1)));
                    if hg_on
                        dist2cent = normvec(g)+1;
                        ...
                        endpt_up_nt = mean(squeeze(ellt_up(:,:,nt)).',1);
                        ellt_up_means = squeeze(mean(ellt_up,2)).';
                        ellt_up_dists = distfun(ellt_up_means,repmat(endpt_up_nt,size(ellt_up_means,1),1));
                        ellt_up_dists(end) = Inf;
                        [~,endpt_up2_ind] = min(abs(ellt_up_dists - dist2cent - arrowWL(2)));
                        if endpt_up2_ind < 2
                            endpt_up2_ind = 2;
                        end
                        endpt_up2 = mean(squeeze(ellt_up(:,:,endpt_up2_ind)).',1);
%                         endpt_up2 = mean(squeeze(ellt_up(:,:,ceil(0.9*nt))).',1);
                        arrow_up2 = [arrow_up2; endpt_up2];
                        endpt_up1 = mean(squeeze(ellt_up(:,:,endpt_up2_ind-1)).',1);
%                         endpt_up1 = squeeze(ellt_up(:,:,floor(0.45*nt))).';
%                         endpt_up1 = mean(squeeze(ellt_up(:,:,floor(0.85*nt))).',1);
                        arrow_up1 = [arrow_up1; endpt_up1];
                        arrow_up_wls = [arrow_up_wls; [log(1.25+normmat(h,g))*arrowWL(1), arrowWL(2)]];
...
%                         tipang_up = [tipang_up;(25*(conmat(g,h)/max(conmat(:)))+20)];
                    ...
                    if gh_on
                        ...
                        endpt_dn_nt = mean(squeeze(ellt_dn(:,:,nt)).',1);
                        ellt_dn_means = squeeze(mean(ellt_dn,2)).';
                        ellt_dn_dists = distfun(ellt_dn_means,repmat(endpt_dn_nt,size(ellt_dn_means,1),1));
                        ellt_dn_dists(end) = Inf;
                        [~,endpt_dn2_ind] = min(abs(ellt_dn_dists - dist2cent - arrowWL(2)));
                        if endpt_dn2_ind < 2
                            endpt_dn2_ind = 2;
                        end
                        endpt_dn2 = mean(squeeze(ellt_dn(:,:,endpt_dn2_ind)).',1);
%                         endpt_dn2 = mean(squeeze(ellt_dn(:,:,ceil(0.9*nt))).',1);
                        arrow_dn2 = [arrow_dn2; endpt_dn2];
                        endpt_dn1 = mean(squeeze(ellt_dn(:,:,endpt_dn2_ind-1)).',1);
%                         endpt_dn1 = mean(squeeze(ellt_dn(:,:,floor(0.85*nt))).',1);
                        arrow_dn1 = [arrow_dn1; endpt_dn1];
                        arrow_dn_wls = [arrow_dn_wls; [log(1.25+normmat(g,h))*arrowWL(1), arrowWL(2)]];
...
%         aup = arrow(arrow_up1(:,[2,1,3]),arrow_up2(:,[2,1,3]),'CrossDir',crossdir_up(:,[2 1 3]),'TipAngle',45,'Length',5);
            aup = arrow3(arrow_up1(:,[2,1,3]),(arrow_up2(:,[2,1,3])-arrow_up1(:,[2,1,3])),'0',arrow_up_wls(:,1),arrow_up_wls(:,2),'cone'); hold on;
...
%         adn = arrow(arrow_dn1(:,[2,1,3]),arrow_dn2(:,[2,1,3]),'CrossDir',crossdir_dn(:,[2 1 3]),'TipAngle',45,'Length',5);
            adn = arrow3(arrow_dn1(:,[2,1,3]),(arrow_dn2(:,[2,1,3])-arrow_dn1(:,[2,1,3])),'0',arrow_dn_wls(:,1),arrow_dn_wls(:,2),'cone'); hold on;
...
% %                 plot3(cony.',conx.',conz.','Color',curcmap,'LineWidth',conwidth); hold on;
    ...
    if input_struct.regsUbins
        ...
        for i = 1:nbin
            ...
            colors_xyz = zeros(size(xyz_jitter,1),3);
            for j = 1:length(x)
                color_xyz_inds = j:size(xyzmap,1):(j+(floor(ifac))*(size(xyzmap,1)));
                cmapind = input_struct.region_groups(brainat(x(j),y(j),z(j)));
                colors_xyz(color_xyz_inds,:) = repmat(cmap(cmapind,:),floor(ifac)+1,1);
            end
            ptcloud = pointCloud(xyz_jitter,'Color',colors_xyz,...
                'Intensity',repmat(0.1*i,size(xyz_jitter,1),1));
            pcshow(ptcloud,'MarkerSize',ptsz); hold on;
            clear x y z xyzmap ptcloud xyz_jitter color_xyz
        end
    else
        interpmap = input_struct.data;
        if size(interpmap,2) <= 1
...
if ~strcmp(imgtype(1:2),'-d')
    imgtype = ['-d' imgtype];
end
imgdir = input_struct.img_directory;
imgrender = input_struct.img_renderer;
if ~strcmp(imgdir(end),filesep)
    imgdir = [imgdir filesep];
end
imgviews = input_struct.img_views;
set(gcf, 'InvertHardCopy', 'off'); 
if savenclose
    for i = 1:size(imgviews,1)
        view(imgviews(i,:));
        ax = gca;
        set(ax,'XColor','none','YColor','none','ZColor','none');
        set(ax,'XTick',[],'YTick',[],'ZTick',[]);
        set(gcf,'renderer',imgrender)
        if isequal(abs(imgviews(i,:)),[0 0 1])
            print([imgdir imglab '_sagittal'],imgtype,'-r600'); 
        elseif isequal(abs(imgviews(i,:)),[0 1 0])
            print([imgdir imglab '_coronal'],imgtype,'-r600'); 
        elseif isequal(abs(imgviews(i,:)),[1 0 0])
            print([imgdir imglab '_axial'],imgtype,'-r600'); 
        else
            imgstr = ['_view' num2str(i)];
            print([imgdir imglab imgstr],imgtype,'-r600'); 
        end
    end
    close
    clear isomap
